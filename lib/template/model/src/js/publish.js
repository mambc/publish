$(function () {
	...
	function initMap() {
		...
		var proporcaoLayer = ((($('#map').height()-80)*40)/100);
		var proporcao = ((($('#map').height()-80)*20)/100);
		
		if ($("#layers").height() < proporcaoLayer)
			proporcao = ((($('#map').height()-80)*30)/100);
				
		$("#layers").css('max-height', proporcaoLayer);
		$("#properties").css('max-height',proporcao);
		$("#legends").css('max-height',proporcao);
