$(function () {
	...
	function loadInitialData(dataset) {
		...
		$.each(dataset, function (id, selected) {
			initLabel(selected);
			if (!selected.visible && !selected.time) {
				return true;
			}
			...
			if (timeStrategy) {
				...
				if (timeStrategy === timeCreation) {
					...
					XHRs.push($.getJSON(url, function (geojson) {
						...
						if (selected.visible) setCheckbox(id, true);
					}));
				} else {
					$.each(selected.name, function (_, temporalLayer) {
				...
				if (!hasTemporalLayer && selected.visible) hasTemporalLayer = true;
 			} else {
	...
	function changeSlide(event, ui) {
		...
		$.each(timelineLayers, function(idx, item) {
