			...
			if (item.geom === "WMS") {
				...
				if (item.time === "snapshot") {
					...
					$.each(item.name, function(idx, itemLayer) {
						...
						var exist = existYear(item.timeline[i]);
						var position = identifyTimelinePosition(item.timeline[i]);
							...
							arrayLayer.push( 
									{
										type: index,
										layer: layerTime,
										year: item.timeline[i]
									});
						}
						else {
							timelineYears.splice(position, 0, item.timeline[i]);
							arrayLayer.push( 
								{
									type: index,
									layer: layerTime,
