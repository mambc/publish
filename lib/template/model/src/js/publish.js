$(function () {
			...
			if (item.geom === "WMS") {
				if (item.time === "snapshot") {
					...
					$.each(item.name, function(idx, itemLayer) {
						...
						timelineValues(item.timeline[i], layerTime, index);
				...
				if (item.time === "creation"){
					...
					var geoJson = $.getJSON(url, function (geojson) {
						...
						for (var i = 0; i <= item.timeline.length - 1; i++) {
							timelineValues(item.timeline[i], mdata, index);
						}	
					});
	...
	function timelineValues(year, layer, index) {
		var exist = existYear(year);
		var position = identifyTimelinePosition(year);
		
		if (exist) {
			addArrayLayer(index, layer, year);
		}
		else {
	...
	function addArrayLayer(type, layer, year) {
		arrayLayer.push( 
				{
					type: type,
					layer: layer,
