$(function () {
	...
	function setFeatureProperties(feature, selected) {
	...
	function initMap() {
		...
		var arrayGeoJson=[];
		enableLoaderMessage();
		$.each(Publish.data, function(index, item) {
			if (item.geom === "WMS") {
				if (item.time === "snapshot") {
					var i = 0;
					$.each(item.name, function(idx, itemLayer) {
						layerTime = new geoambiente.maps.WmsLayer({
							url: item.url,
							layer: itemLayer,
						...
						timelineValues(item.timeline[i], layerTime, index, item.time);
			...
			{
				if (item.time === "creation"){
					var i = 0;
					
					var url = Publish.path + item.name + ".geojson";
					var geoJson = $.getJSON(url, function (geojson) {
						var mdata = new google.maps.Data();
						mdata.addGeoJson(geojson);
						
						setDataProperties(mdata, item);
						mdata.setMap(map);
						
						for (var i = 0; i <= item.timeline.length - 1; i++) {
							timelineValues(item.timeline[i], mdata, index, item.time);
						}	
					});
				...
				if (item.time === "snapshot") {
					var i = 0;
					$.each(item.name, function(idx, itemLayer) {
						var url = Publish.path + itemLayer + ".geojson";
						var geoJson = $.getJSON(url, function (geojson) {
							var mdata = new google.maps.Data();
							mdata.addGeoJson(geojson);
		...
		$.when.apply($, arrayGeoJson).always(function () {
			$("#slider").slider({
				range: true,
				min: 0,
	...
	function changeSlide(event, ui) {	
		...
		$.each(arrayLayer, function(idx, item) {
			if (item.time === 'snapshot') {
				if (item.year === timelineYears[ui.values[1]])
			...
			if (item.time === 'creation') {
				item.layer.setStyle(function(feature){
				    if (feature.getProperty("anoCriacao") >= timelineYears[ui.values[0]] && feature.getProperty("anoCriacao") <= timelineYears[ui.values[1]]) {
				       return ({visible: true});
				    }
				    else
				    	 return ({fillColor: 'white', strokeColor: 0, strokeOpacity:0 });
				});
			}
		});
	}
	...
	function timelineValues(year, layer, index, time) {
		var exist = existYear(year);
		var position = identifyTimelinePosition(year);
		
		if (exist) {
			addArrayLayer(index, layer, year, time);
		}
		else {
	...
	function addArrayLayer(type, layer, year, time) {
		arrayLayer.push( 
				{
					type: type,
					layer: layer,
