$(function () {
	...
	function createTimeline() {
		...
		if (vals === 0) {
			...
			var el = $('<label> </label>').css('left', '0%');
			$slider.append(el);

			el = $('<label>' + timelineYears[0] + '</label>').css('left', '100%');
			$slider.append(el);
		} else {
	...
	function onScenarioChange() {
		...
		var scenarioName = $scenarioCombobox.find('option:selected').text();
		...
		if (view) {
			...
			if (inCache) {
				...
				enableLoaderMessage();
				$loaderMessageText.empty();
				addLoaderMessage(scenarioName, 1, 1);
