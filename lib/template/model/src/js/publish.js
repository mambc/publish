$(function () {
	...
	var $scenario = $('#scenario-combobox');
	...
	function loadData(layer, selected, __sof__) {
		...
		if (selected.geom === "WMS") {
			...
			data[layer] = mdata;
			if (selected.visible) setCheckbox(viewInCheckBox, true);
	...
	function changeLayer(layer, selected, temporal) {
		selected = selected || Publish.data[layer];
		if (selected.time && !temporal) {
			$.each(selected.name, function (_, temporalLayer) {
				changeLayer(temporalLayer, selected, true);
			});
		} else {
			...
			if (mdata) {
				if (isLineStringOrMultiLineString(selected)) {
					...
					var viewInCheckBox = (selected.time) ? selected.id : layer;
					if (selected.geom === GEOMETRY.WMS) {
						if (mdata.getVisibility()) {
							removeLayer(mdata, viewInCheckBox);
						} else {
							addLayer(mdata, viewInCheckBox);
						}
					} else if (mdata.getMap()) {
						removeLayer(mdata, viewInCheckBox);
					} else {
						addLayer(mdata, viewInCheckBox);
					}
	...
	function loadInitialData(dataset) {
		...
		$.when.apply($, XHRs).always(function () {
			if (hasTemporalViewVisible && hasTemporalView) {
				createSlider();
				scenarioDefaultData.timelineYears = clone(timelineYears);
				scenarioDefaultData.timelineLayers = clone(timelineLayers);
			}
	...
	function setLayerVisibility(yearIndex) {
		$.each(timelineLayers, function (_, item) {
			...
			if (item.scenario && !scenarioActive[item.scenario]) {
				gmap = null;
			}
	...
	function changeSlide(event, ui) {
		...
		$spanSelectedYear.css('left', (ui.values[1] / vals * 100) + '%');
	...
	function createTimeline() {
		var vals = timelineYears.length - 1;
		var x = 0;
		selectedInterval = [0, timelineYears.length - 1];
		$.each(timelineYears, function (idx, item) {
			var el = $('<label>' + (item) + '</label>').css('left', (x / vals * 100) + '%');
			$slider.append(el);
			x++;
		});
	}
	...
	function removeCurrentScenarioIfExist() {
		if (scenarioDefaultData.currentScenario) {
			var currentView = Publish.data[scenarioDefaultData.currentView];
			var currentScenario = currentView.scenario[scenarioDefaultData.currentScenario];
			$.each(currentScenario.name, function (_, layer) {
				var mdata = data[layer];
				mdata.setMap(null);
			});

			$.each(currentScenario.timeline, function (_, year) {
				var index = timelineYears.indexOf(year);
				if (index >= 0) {
					timelineYears.splice(index, 1);
				}
			});

			scenarioActive[scenarioDefaultData.currentScenario] = false;
		}
	}

	function onScenarioChange() {
		removeCurrentScenarioIfExist();
		var scenarioName = $scenario.find('option:selected').text();
		var view = $scenario.find('option:selected').val();
		if (view) {
			var scenarioData = scenarioActive[scenarioName];
			if (scenarioData) {
				$slider.find('label').remove();
				// timelineYears = scenarioData.timelineYears;
				// timelineLayers = scenarioData.timelineLayers;
				createSlider();
			} else {
				var XHRs = [];
				...
				// timelineYears = scenarioDefaultData.timelineYears;
				// timelineLayers = scenarioDefaultData.timelineLayers;
				...
				$.when.apply($, XHRs).always(function () {
					...
					scenarioActive[scenarioName] = true;
				});
			}

			scenarioDefaultData.currentScenario = scenarioName;
			scenarioDefaultData.currentView = view;
		} else {
			removeCurrentScenarioIfExist();
			$slider.find('label').remove();
			timelineYears = scenarioDefaultData.timelineYears;
			timelineLayers = scenarioDefaultData.timelineLayers;
			scenarioDefaultData.currentScenario = null;
			scenarioDefaultData.currentView = null;
			createSlider();
		}
	}
