$(function () {
	...
	function onGroupsClick(event) {
		var name = event.target.id.replace("group-", "");
		...
		if (!menu.group) {
			menu.group = name;
			$.each(groups, function (gp) {
				if (gp === name) return;
				$('#group-' + gp).prop('disabled', true);
			});
		} else if (menu.group === name) {
			menu.group = null;
			$.each(groups, function (gp) {
				$('#group-' + gp).prop('disabled', false);
			});
		}
	}
	...
	function startMenu() {
		...
		var $groups = $panelLayers.find(':link');
		$groups.click(onGroupsClick);
	}
