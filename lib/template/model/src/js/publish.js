$(function () {
	...
	function loadInitialData(dataset) {
		...
		$.each(dataset, function (id, selected) {
			...
			if (selected.time) {
				if (selected.time === timeCreation) {
					throw Error("Time 'creation' not implemented yet.");
				}
				...
				$.each(selected.name, function (_, temporalLayer) {
					...
					XHRs.push(loadData(temporalLayer, selected, function (gdata) {
						timelineValues(year, gdata, id, selected.time);
					}));

					timelineIndex ++;
				});

				if (!hasTemporalLayer) hasTemporalLayer = true;
 			} else {
